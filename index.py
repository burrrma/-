...
# ФУНКЦИЯ ДЛЯ ПАРСИНГА ТАБЛИЦЫ ПО ССЫЛКЕ БАКАЛАВРИАТА И МАГИСТРАТУРЫ ОЧНОЙ ФОРМЫ ОБУЧЕНИЯ
def connect_to_tables(link, SAMPLE_RANGE_NAME, course, form):
  ...
  """
    ...
    try:
        ...
        print(err)
        text = "Oшибка доступа к серверу, давайте я расскажу вам последние новости"
...
def table_parsing_OZ(link, course, form) -> list:
    SAMPLE_RANGE_NAME = "A11:H1000"
    values = connect_to_tables(link, SAMPLE_RANGE_NAME, course, form)
...
def table_parsing(link, course, form) -> tuple[list[dict[Any, dict[Any, Any]]], list[Any]]:
    try:
        if link is None:
            return None, None
        """
        ...
        pattern_of_auditorium = r'\d{3}|(\s*\d*\w*)\d{3}(\s*\d*\w*)|online|\d{3}[\d\s]*\d{3}|^\d{3}\s(?:Львов|Сорм|Кост)$|Online'
        ...
        for elem in values:
            ...
            for i in range(0, len(elem)):
                ...
                if (elem[i] == "" or elem[i] == "Военная кафедра") and i > 2:
...
def lessons_split(schedule_res: list, students_groups: list):
    try:
        if (schedule_res is None):
            return None
        week_days = ["понедельник", "вторник", "среда", "четверг", "пятница", "суббота"]
        pattern_of_lecturer = r"\w+\s\w\.\w\.|\w+\s\w\.\w|\w+\s\w\.|\w\.\w\.\s\w+|[А-ЯЁ]\w+ [А-ЯЁ]\w+ [А-ЯЁ]\w+"
...
def start_rasp_with_weekday(event):
    ...
    if form == "ochnozaochno":
        ...
        schedule_arr = transform_to_classes(lessons_split(table_parsing(link, curs, form)[0],
                                                          table_parsing(link, curs, form)[1]))
    ...
    try:
        ...
        if event['session']['new']:
            ...
            if (napravlenie is not None and curs is not None and level is not None and form is not None):
                ...
                schedule_arr = []
                if form == "ochnozaochno":
                    schedule_arr_OZ = table_parsing_OZ(link, curs, form)
                elif form == "ochno":
                    schedule_arr_OZ = []
            ...
            txt = "Привет! Я подскажу расписание ВШЭ и покажу новости. Чтобы подробнее узнать о том, что я умею, ты можешь попросить меня показать справку."
