...
#ФУНКЦИЯ ДЛЯ ПАРСИНГА ТАБЛИЦЫ ПО ССЫЛКЕ БАКАЛАВРИАТА И МАГИСТРАТУРЫ ОЧНОЙ ФОРМЫ ОБУЧЕНИЯ
def connect_to_tables(link, SAMPLE_RANGE_NAME):
...
def table_parsing_OZ(link) -> list:
    SAMPLE_RANGE_NAME = "A11:H1000"
    values = connect_to_tables(link, SAMPLE_RANGE_NAME)
    ...
    for i in range(nn,len(values)):
        ...
        if len(values[i]) == 8:
            ...
            sp_ = [dat_, dat_.weekday(), values[i][1], values[i][3], values[i][4], values[i][5], values[i][6], values[i][7]]
            spisok.append(sp_)
    return spisok
...
def table_parsing(link) -> tuple[list[dict[Any, dict[Any, Any]]], list[Any]]:
    if link is None:
        return None, None
    """
    ...
    # переработка строк, которая убирает дефисы и переводы строки
    for i in range(len(values)):
...
def lessons_split(schedule_res: list, students_groups: list):
    if (schedule_res is None):
        return None  # функция для разделения нескольких пар, не допилил
    # включить strip для пробелов
    week_days = ["понедельник", "вторник", "среда", "четверг", "пятница", "суббота"]
    pattern_of_lecturer = r"\w+\s\w\.\w\.|\w+\s\w\.\w|\w+\s\w\."
        ...
        for cur_week_day in week_days:
            ...
            for j in range(len_of_lessons_arr):  # пробуем изменить длину и взять как переменную тут гле-то ошибка
                if len((schedule_res[i][cur_group][cur_week_day]["lessons"][j][
                    "auditorium"]).split()) != 1:  # иногда 1 аудитория и поэтому неправильно парсит
    ...
    for i in range(len(students_groups)):  # разбиваем на преподов и типы предметов те ячейки, где один предмет
        ...
        for cur_week_day in week_days:
            for j in range(len(schedule_res[i][cur_group][cur_week_day]["lessons"])):
                if len((schedule_res[i][cur_group][cur_week_day]["lessons"][j][
                    "auditorium"]).split()) == 1:  # вставил второе условие мб поломается
...
def poisk_lector_M_B(schedule_, param_):
    ...
    for i in range(6):
        ...
        for j in range(len(eval(str_poisk))):
            ...
            if eval(str_poisk_l).lower().find(param_.lower()) != -1: #поменял if eval(str_poisk_l).lower().find(param_.lower()) > 0: 
                answer = week_days[(dt_week + i) % 6] + " " + eval(str_poisk + "[" + str(j) + "].time") + "\n"
                answer += eval(str_poisk_l).replace("\n", " ")
                return answer
...
def start_rasp_with_weekday(event):
    ...
    if form == "ochnozaochno":
        ...
        schedule_arr = transform_to_classes(lessons_split(table_parsing(link)[0],
                            table_parsing(link)[1]))
...
def handler(event, context):
    ...
    if event['session']['new']:
        ...
        return make_response(
            text = txt, tts = "Привет! <speaker audio='dialogs-upload/05d0273d-97bb-45de-a2c5-52b00fa2ec70/54db7e77-f45c-457e-9b8a-143a0515d47c.opus'> sil <[1500]> Я подскажу расписание Высшей Школы Экономики. Чтобы подробнее узнать о том, что я умею, ты можешь попросить меня показать справку.",
        ...
        if (napravlenie is None or curs is None or group is None or level is None):
            ...
            return start_rasp_sub(event) #запускается Настина функция
        ...
        if(form == 'zaochno'):
            return make_response(text = "Ой! Я пока не умею работать с заочным форматом обучения. Я не могу подсказать расписание, но зато могу рассказать интересные новости! Чтобы послушать новость, скажи: Покажи новости")
        ...
        return make_response(text='Внимательно запоминаю информацию. Последний вопрос - в какой группе ты учишься?',
                             user_state_update={'napravlenie_for_search': napravlenie, 'napr_text': napr_text,
                                                ...
                                                'link': link, 'schedule_arr_OZ' : schedule_arr_OZ})  # если в запросе есть направление
